    ...
    <div class="project-list">
        ...
        <div v-for="p in filtered" :key="p.id" class="project-row">
            <div class="col-name">
                ...
                <div v-if="p.tags?.length" class="project-tags">
            ...
            <div class="col-target">
                <div class="target-list">
                    <span v-for="target in p.target?.slice(0, 3)" :key="target" class="target-item">
                        {{ target }}
                    </span>
                    <span v-if="(p.target?.length || 0) > 3" class="target-more">
                        +{{ (p.target?.length || 0) - 3 }}
                    </span>
                </div>
            </div>
            ...
            <div class="col-links">
                <div class="link-buttons">
                    <a v-if="p.repos?.length" :href="p.repos[0].url" target="_blank" rel="noopener"
                        class="link-btn repo">Repo</a>
                    <a v-if="p.links?.docs" :href="p.links.docs" target="_blank" rel="noopener"
                        class="link-btn docs">Docs</a>
                    <a v-if="p.links?.issues" :href="p.links.issues" target="_blank" rel="noopener"
                        class="link-btn issues">Issues</a>
                </div>
            </div>
        </div>
    </div>
